...
const Todos = ({ todos, deleteTodo }) => {
  const todoList = todos.length ? (
    todos.map((todo) => {
      const backgroundcolor = (new Date(todo.due) < new Date()) ? "rgb(255, 140, 190)" : "white"
      const color = (new Date(todo.due) < new Date()) ? "white": "rgb(255, 140, 190)"
      return (
        <Grid key={todo.id}>
          <Card style={{ color: color, padding: "10px 0px", margin: "10px", borderRadius: "20px", backgroundColor: backgroundcolor}} data-testid={todo.content}>
